<%@ Page Title="" Language="C#"  AutoEventWireup="true" CodeBehind="SelectProduct.aspx.cs" Inherits="Beeant.Presentation.Admin.Erp.Order.Order.SelectProduct" %>
    ...
    <ContentTemplate>
       ...
       <Columns>
            ...
            <asp:TemplateField ItemStyle-CssClass="center ckbox">
                ...
                <ItemTemplate>
                     <input value='<%#Eval("IsReturn").Convert<bool>().ToString().ToLower() %>' id="hfIsReturn"  type="hidden" runat="server"   SerializeName="IsReturn" />
                    <input value='<%#GetDescription(Eval("Goods").Convert<GoodsEntity>(),Eval("Id").Convert<long>()) %>' id="hfDescription" type="hidden" runat="server"  SerializeName="Description" />
                     <input value='<%#Eval("Name") %>' id="hfName" type="hidden" runat="server"  SerializeName="Name" />
                      <input value='<%#Eval("Price") %>' id="hfPrice" type="hidden" runat="server" SerializeName="Product_Price" />
                       <input value='<%#Eval("Cost") %>' id="hfCost" type="hidden" runat="server" SerializeName="Product_Cost" />
                       <input value='<%#Eval("Count") %>' id="hfCount" type="hidden" runat="server" SerializeName="Product_Count" />
                        <input value='<%#Eval("Id") %>' id="hfProductId" type="hidden" runat="server" SerializeName="Id" />
           ...
           <asp:TemplateField HeaderText="面价"  ItemStyle-CssClass="left Sequence">
             ...
             <asp:TemplateField HeaderText="商品价格"  ItemStyle-CssClass="left Sequence xlstext">
                                <ItemTemplate>
                                   <input type="text" runat="server" id="txtCost" value='<%#Eval("Cost") %>' SerializeName="Price" count='count' style=""/>
                                </ItemTemplate>
                            </asp:TemplateField>
                                <asp:TemplateField HeaderText="备注"  ItemStyle-CssClass="left Sequence xlstext">
                                <ItemTemplate>
                                   <input type="text" runat="server" id="txtRemark" value='' SerializeName="Remark" style=""/>
                                </ItemTemplate>
                            </asp:TemplateField>
        </Columns>
     </asp:GridView>
        </div>        
        <uc1:Pager ID="Pager1" runat="server" PageSize="10"   SelectExp="Id,Name,Price,Cost,Count,OrderMinCount,IsReturn,IsSales,Goods.GoodsDetails.Select(Name,Detail)" OrderByExp="Id asc"  />
