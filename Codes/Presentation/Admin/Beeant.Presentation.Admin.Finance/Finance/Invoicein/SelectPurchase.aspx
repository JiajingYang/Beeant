<%@ Page Title="" Language="C#"  AutoEventWireup="true" CodeBehind="SelectPurchase.aspx.cs" Inherits="Beeant.Presentation.Admin.Finance.Finance.Invoicein.SelectPurchase" %>
       ...
       <Columns>
        ...
        <asp:TemplateField ItemStyle-CssClass="center ckbox">
            ...
            <ItemTemplate>
                    ...
                    <input value='<%#Eval("OpenAmount") %>' id="hfAmount" type="hidden" runat="server" SerializeName="OpenAmount" />
                    <input value='<%#Eval("InvoiceAmount") %>' id="hfInvoiceAmount" type="hidden" SerializeName="InvoiceAmount" />
                    <input value='<%#Eval("InsertTime", "{0:yyyy-MM-dd HH:mm}")%>' id="hfInsertTime" type="hidden"  SerializeName="InsertTime" />
               <input value='<%#Eval("Id") %>' id="ckSelect"  type="checkbox" <%# Convert.ToDouble(Eval("Amount").ToString())-Convert.ToDouble(Eval("InvoiceAmount").ToString())>0?"checked='checked'":"" %>  SubCheckName="selectall" ComfirmValidate="Remove, Price" SerializeName="Id" />
           </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="本次开票"  ItemStyle-CssClass="left">
            <ItemTemplate>
                <input value='<%# Convert.ToDouble(Eval("OpenAmount").ToString())-Convert.ToDouble(Eval("InvoiceAmount").ToString()) %>' id="txtAmount" runat="server" type="text" class="input"  SerializeName="Amount" />                
            </ItemTemplate>
        </asp:TemplateField> 
        ...
        <uc1:Pager ID="Pager1" runat="server" PageSize="10"   SelectExp="Id,Status,Level,User.RealName,Submit.RealName,InsertTime,OpenAmount,InvoiceAmount" OrderByExp="Id asc"  />
