<%@ Page Title="" Language="C#"  AutoEventWireup="true" CodeBehind="SelectPurchase.aspx.cs" Inherits="Beeant.Presentation.Admin.Finance.Finance.Payout.SelectPurchase" %>
       ...
       <Columns>
        ...
        <asp:TemplateField ItemStyle-CssClass="center ckbox">
            ...
            <ItemTemplate>
                    ...
                    <input value='<%#Eval("TotalAmount") %>' id="hfTotalAmount" type="hidden" runat="server" SerializeName="TotalAmount" />
                    <input value='<%#Eval("PayAmount") %>' id="hfPayAmount" type="hidden" SerializeName="PayAmount" />
                    <input value='<%#Eval("InsertTime", "{0:yyyy-MM-dd HH:mm}")%>' id="hfInsertTime" type="hidden"  SerializeName="InsertTime" />
               <input value='<%#Eval("Id") %>' id="ckSelect"  type="checkbox" <%# Convert.ToDouble(Eval("Amount").ToString())-Convert.ToDouble(Eval("PaidAmount").ToString())>0?"checked='checked'":"" %>  SubCheckName="selectall" ComfirmValidate="Remove, Price" SerializeName="Id" />
           </ItemTemplate>
        </asp:TemplateField>
